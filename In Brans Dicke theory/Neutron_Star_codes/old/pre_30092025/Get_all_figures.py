...
# make_gamma_beta_plots(50,1, w=40000)
...
def plot_w_vs_rho(lowest_w, highest_w, n, count):
    ...
    if count==0:
        ...
        for i, w in enumerate(w_values):
            # print(i)
            for j, d in enumerate(den_space):
                # print(j,d)
                # W.append(w)          # répète w pour chaque densité
                # rho.append(d)          # densité correspondante
                Gamma_EDD.append(gamma_edd_all[i][j])  # gamma correspondant

        # print("Gamma_EDD", Gamma_EDD, len(Gamma_EDD))
        # print('w', w_values, len(w_values))
        ...
        Gamma_EDD_array = np.array(gamma_edd_all)  # shape (len(w_values), len(den_space))
        Gamma_deviation_array = np.array(gamma_deviation_all)

        np.save(all_a, [gamma_edd_all, gamma_BD_all, gamma_diff_all, gamma_deviation_all])
        np.save(all_w, [w_values])
        np.save(all_rho, [den_space])
        np.save(all_gamma, [Gamma_EDD_array, Gamma_deviation_array])


        # First plot only gamma_e

        plt.figure(figsize=(8,6))
        mesh = plt.pcolormesh(den_space, w_values, Gamma_EDD_array, shading='auto', cmap='viridis')
        ...
        plt.figure(figsize=(8,6))
        mesh = plt.pcolormesh(den_space, w_values, 1-Gamma_EDD_array, shading='auto', cmap='viridis')
        cbar = plt.colorbar(mesh)
        ...
        plt.figure(figsize=(8,6))
        mesh = plt.pcolormesh(den_space, w_values, Gamma_deviation_array, shading='auto', cmap='viridis')
        cbar = plt.colorbar(mesh)
        ...
        Gamma_EDD_array = np.load(all_gamma, allow_pickle=True)[0]
        Gamma_deviation_array = np.load(all_gamma, allow_pickle=True)[1]
        gamma_BD_all = np.load(all_a, allow_pickle=True)[1]

        # print(Gamma_EDD_array[140])
        # print(gamma_BD_all[140])
        # print(w_values[140])

        #
        # print(Gamma_EDD_array[-1])
        # print(w_values[-1])
        # print(den_space[-1])
        # print(w_values[2])
        plt.figure(figsize=(8,6))
...
def recover_and_plot(n, lowest_w, highest_w):
    ...
    os.makedirs(save_dir, exist_ok=True)
    all_a = os.path.join(save_dir, f'matrice_{n}.npy')
    # all_plot = os.path.join(save_dir, f'matrice_{n}_plot.npy')
    # all_gamma = os.path.join(save_dir, f'matrice_{n}_gamma.npy')
    if os.path.exists(all_a):
            # gamma_edd_all, gamma_BD_all, gamma_diff_all, gamma_deviation_all = np.load(all_a, allow_pickle=True)
            # W, rho = np.load(all_plot, allow_pickle=True)
            # Gamma_EDD_array = np.load(all_gamma, allow_pickle=True)
            count=1
...
recover_and_plot(n=300, lowest_w = np.log(1e-1),highest_w = np.log(1e6))
...
# plt.figure(figsize=(8,6))
# mesh = plt.pcolormesh(den_space, w_values, Gamma_EDD_array, shading='auto', cmap='viridis')
# plt.colorbar(mesh, label=r'$\gamma$')
# plt.xlabel(r'Densité $\rho$ (MeV/fm$^3$)')
# plt.ylabel(r'$\omega$')
# plt.savefig(f'./saved_matrices_and_plots/heatmap_{n}.png', dpi=200, bbox_inches="tight")
