...
def run_case(init_density_kg_m3: float, option: int, name: str, radiusMax_in: float = 3.0e4, Npoint: int = 1000000):
    tov = TOV(
...
def main():
    ...
    results = []
    for option, name in cases:
        res = run_case(init_density, option, name)
        results.append(res)
    ...
    plt.figure()
    for res in results:
        plt.plot(res["r_in_km"], variation, label=res["name"])
    plt.xlabel("r (km)")
